...
"""
function target_node(ts::AgentState; size=0.15, fill_color="red", stroke_color="black")::Context
  compose(context(), fill(fill_color), stroke(stroke_color),
...
function render_scene(m::HSModel, s::HSState)
  ...
  human_viz = agent_node(s.human_pose, fill_color="tomato")
  target_viz = target_node(s.human_target)

  composition = compose(mirror, (base_scale, human_viz, target_viz, room_viz))
  composition |> SVG("display.svg", 14cm, 14cm)
  return composition
end
