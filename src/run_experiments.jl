...
function main()
    ...
    data = parallel_sim(101:500, [k for k in keys(solver_setup_span_map(keys_only=true))], simulation_hbm_keys=["HumanMultiGoalBoltzmann_all_goals"], planner_hbm_keys=["HumanMultiGoalBoltzmann_all_goals"])
    @info "Writing data..."
